...
command_invocation  = cindent:space* comm:identifier aindent:(space*) '(' args:arguments ')' space*
                        {return {t:"command",cindent:cindent.join(''),name:comm,args:args,aindent:aindent.join('')} } 
// returns the whole identifier as string
identifier          =  iden:( [A-Za-z_][A-Za-z0-9_]* ) {return `${iden[0]}${iden[1].join('')}`}
arguments           =  argument? separated_arguments* {return ''} // temporarily disabled for debugging
